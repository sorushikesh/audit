...
class TtlCleanupSchedulerContainerTest {
  ...
  void removeExpiredDocumentsWithContainer() {
    ...
    template
        .getDb()
        .getCollection(Constants.COLLECTION_JV_SNAPSHOT)
        .insertOne(new Document(Constants.FIELD_TTL_DATE, past));
    ...
    long commitCount =
        template
            .getDb()
            .getCollection(Constants.COLLECTION_COMMIT_METADATA_COLLECTION)
            .countDocuments();
    long snapshotCount =
    ...
    verify(template, times(1))
        .remove(any(Query.class), eq(Constants.COLLECTION_COMMIT_METADATA_COLLECTION));
